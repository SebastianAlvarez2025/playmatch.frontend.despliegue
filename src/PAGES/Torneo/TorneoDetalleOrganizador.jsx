import { useParams } from "react-router-dom";
import { useState, useEffect } from "react";

import { getTorneoById } from "../../services/torneoService";

import Encuentros from "../Encuentros/Encuentros";
import Posiciones from "../Posiciones/Posiciones";
import Resultados from "../Resultados/Resultados";
import Equipos from "../Equipos/Equipos";
import InscripcionesEquiposOrganizador from "../InscripcionesEquipo/InscripcionesEquiposOrganizador";

import trofeo from "../../ASSETS/trofeo.jpg";
import styles from "./TorneoDetalleOrganizador.module.css";

export default function TorneoDetalleOrganizador() {
  const { id } = useParams();

  const [torneo, setTorneo] = useState(null);
  const [tab, setTab] = useState("encuentros");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  // ==========================================================
  // CARGAR TORNEO
  // ==========================================================

  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError("");

        const torneoData = await getTorneoById(id);

        setTorneo(Array.isArray(torneoData) ? torneoData[0] : torneoData);
      } catch (err) {
        console.error("ERROR AL CARGAR TORNEO:", err);

        setError("Error al cargar el torneo");
      } finally {
        setLoading(false);
      }
    };

    fetchData();
  }, [id]);

  // ==========================================================
  // CARGANDO
  // ==========================================================

  if (loading) {
    return <p className={styles.loadingText}>Cargando torneo...</p>;
  }

  // ==========================================================
  // ERROR
  // ==========================================================

  if (error) {
    return <p className={styles.errorText}>{error}</p>;
  }

  // ==========================================================
  // TORNEO NO ENCONTRADO
  // ==========================================================

  if (!torneo) {
    return <p className={styles.notFoundText}>No se encontró el torneo</p>;
  }

  // ==========================================================
  // VISTA
  // ==========================================================

  return (
    <div className={styles.detalleContainer}>
      {/* ======================================================
          HEADER DEL TORNEO
      ====================================================== */}

      <header className={styles.detalleHeader}>
        <h1>{torneo.nombre_torneo}</h1>

        <div className={styles.dataDetalle}>
          {/* TROFEO */}

          <div className={styles.imageContainer}>
            <img src={trofeo} alt="Logo del torneo" />
          </div>

          {/* INFORMACIÓN */}

          <div className={styles.info}>
            <p>
              <span className={styles.torneoSubtitulo}>Tipo De Torneo:</span>{" "}
              {torneo.tipo_torneo}
            </p>

            <p>
              <span className={styles.torneoSubtitulo}>Ciudad:</span>{" "}
              {torneo.ciudad}
            </p>

            <p>
              <span className={styles.torneoSubtitulo}>Estado:</span>{" "}
              {torneo.estado}
            </p>

            <p>
              <span className={styles.torneoSubtitulo}>
                Fecha de Finalización:
              </span>{" "}
              {torneo.fecha_fin
                ? new Date(torneo.fecha_fin).toLocaleDateString("es-CO")
                : "—"}
            </p>
          </div>
        </div>

        {/* ====================================================
            TABS
        ==================================================== */}

        <nav className={styles.detalleNav}>
          {/* POSICIONES */}

          <button
            className={tab === "posiciones" ? styles.activeTab : ""}
            onClick={() => setTab("posiciones")}
          >
            Posiciones
          </button>

          {/* ENCUENTROS */}

          <button
            className={tab === "encuentros" ? styles.activeTab : ""}
            onClick={() => setTab("encuentros")}
          >
            Encuentros
          </button>

          {/* RESULTADOS */}

          <button
            className={tab === "resultados" ? styles.activeTab : ""}
            onClick={() => setTab("resultados")}
          >
            Resultados
          </button>

          {/* EQUIPOS */}

          <button
            className={tab === "equipos" ? styles.activeTab : ""}
            onClick={() => setTab("equipos")}
          >
            Equipos
          </button>

          {/* INSCRIPCIONES */}

          <button
            className={tab === "inscripciones" ? styles.activeTab : ""}
            onClick={() => setTab("inscripciones")}
          >
            Inscripciones
          </button>
        </nav>
      </header>

      {/* ======================================================
          CONTENIDO DE LA PESTAÑA
      ====================================================== */}

      <main className={styles.detalleMain}>
        {/* POSICIONES */}

        {tab === "posiciones" && <Posiciones id_torneo={id} />}

        {/* ENCUENTROS */}

        {tab === "encuentros" && <Encuentros id_torneo={id} />}

        {/* RESULTADOS */}

        {tab === "resultados" && <Resultados id_torneo={id} />}

        {/* EQUIPOS */}

        {tab === "equipos" && <Equipos id_torneo={id} />}

        {/* ==================================================
            INSCRIPCIONES DEL TORNEO

            IMPORTANTE:
            InscripcionesEquiposOrganizador recibe:

            idTorneoProp

            y luego ejecuta:

            getInscripcionesPorTorneo(idTorneoFinal)
        ================================================== */}

        {tab === "inscripciones" && (
          <InscripcionesEquiposOrganizador idTorneoProp={id} />
        )}
      </main>
    </div>
  );
}
